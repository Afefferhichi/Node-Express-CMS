...
const applyHelpfuls = (helpfuls, user_id) => {
    ...
    const existingIndex = newHelpfuls.findIndex(helpful_user_id => String(helpful_user_id) === user_id);
    if(existingIndex > -1) {
...
const helpful = (req, res) => {
    try {
        Comment.findById(req.params.id)
            .then((comment) => {
                if (comment) {
                    console.log('Comment helpful', req.user);
                    let helpfulMethod = req.route.path;
                    helpfulMethod = helpfulMethod.substr(helpfulMethod.lastIndexOf('/') + 1)

                    const user_id = String(req.user._id);
                    
                    if (helpfulMethod === 'helpful') {
                        comment.cmtHelpfuls = applyHelpfuls(comment.cmtHelpfuls, user_id);
                        comment.cmtUnHelpfuls = unApplyHelpfuls(comment.cmtUnHelpfuls, user_id);
                    }
                    else if (helpfulMethod === 'unhelpful') {
                        comment.cmtUnHelpfuls = applyHelpfuls(comment.cmtUnHelpfuls, user_id);
                        comment.cmtHelpfuls = unApplyHelpfuls(comment.cmtHelpfuls, user_id);
                    }
